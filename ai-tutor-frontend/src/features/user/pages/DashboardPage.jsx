...
import { fetchRecentChatSessions } from "@/shared/services/api.service";
import { DASHBOARD_WORKSPACE_TEXTS } from "@/shared/constants/texts";
...
function LearningContextSkeleton() {
  return (
    <div className="space-y-2" role="status" aria-label={T.context.loadingTitle} aria-busy="true">
      {[0, 1, 2].map((i) => (
        <div
          key={i}
          className="flex items-center gap-3 rounded-[var(--radius-panel)] border border-[var(--border-subtle)] bg-[var(--surface)] p-3"
        >
          <Skeleton className="h-10 w-10 shrink-0 rounded-[var(--radius-panel)]" />
...
export default function Dashboard() {
  ...
  const [recentSessionsLoading, setRecentSessionsLoading] = useState(!sessionCache.loaded);
  ...
  useEffect(() => {
    let active = true;
    // Show cached data immediately, but always re-fetch for freshness
    if (!sessionCache.loaded) setRecentSessionsLoading(true);
