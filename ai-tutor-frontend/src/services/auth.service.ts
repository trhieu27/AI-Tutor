...
class AuthService {
  ...
  private persistUser(rawUser) {
    try { localStorage.setItem('user', JSON.stringify(rawUser)); } catch {}
  }
  ...
  public async googleLogin(token) {
    const response = await fetch(`${AUTH_BASE}/google-login`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ token }),
    });
    if (!response.ok) { const e = await response.json(); throw new Error(e.detail || 'Google login failed'); }
    const data = await response.json();
    localStorage.setItem('access_token', data.access_token);
    localStorage.setItem('refresh_token', data.refresh_token);
    this.persistUser(data.user);
  ...
  public async register(name, email, password) {
    ...
    if (!response.ok) { const e = await response.json(); throw new Error(e.detail || 'Registration failed'); }
    const data = await response.json();
    localStorage.setItem('access_token', data.access_token);
    localStorage.setItem('refresh_token', data.refresh_token);
    this.persistUser(data.user);
  ...
  public async forgotPassword(email): Promise<void> {
    ...
    if (!response.ok) { const e = await response.json(); throw new Error(e.detail || 'Không thể gửi yêu cầu khôi phục'); }
  }

  public async verifyOtp(email, otp): Promise<void> {
    ...
    if (!response.ok) { const e = await response.json(); throw new Error(e.detail || 'Mã xác nhận không hợp lệ'); }
  }

  public async resetPassword(email, otp, password): Promise<void> {
    const response = await fetch(`${AUTH_BASE}/reset-password`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, otp, new_password: password }),
    });
    if (!response.ok) { const e = await response.json(); throw new Error(e.detail || 'Không thể đặt lại mật khẩu'); }
  }
  ...
  public async getCurrentUser() {
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) { const newToken = await this.refreshToken(); if (newToken) return this.getCurrentUser(); }
        return null;
      }
