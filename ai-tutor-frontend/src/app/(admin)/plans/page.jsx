...
import { fetchAdminPlans, updateAdminPlan } from "@/services/admin.service";
...
function toDraft(plan) {
  return {
    ...
    quota: {
      ...
      max_file_size_mb: plan.quota?.max_file_size_mb ?? 0,
    },
  };
}
    ...
    <article className="rounded-[var(--radius-panel)] border border-[var(--border-color)] bg-[var(--card-bg)] shadow-[var(--premium-shadow-sm)]">
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-2">
        <label className="grid gap-1 text-[11px] font-bold text-[var(--muted)]">
          Tên hiển thị
          <input className="premium-input h-10 px-3" value={draft.display_name} onChange={(event) => update("display_name", event.target.value)} />
        </label>
        <label className="grid gap-1 text-[11px] font-bold text-[var(--muted)]">
          {ADMIN_TEXTS.plans.sortOrder}
          <input type="number" className="premium-input h-10 px-3" value={draft.sort_order} onChange={(event) => update("sort_order", Number(event.target.value))} />
        </label>
        <label className="grid gap-1 text-[11px] font-bold text-[var(--muted)]">
          {ADMIN_TEXTS.plans.price}
          <input type="number" className="premium-input h-10 px-3" value={draft.price_vnd} onChange={(event) => update("price_vnd", Number(event.target.value))} />
        </label>
        <label className="grid gap-1 text-[11px] font-bold text-[var(--muted)]">
          {ADMIN_TEXTS.plans.discountedPrice}
          <input type="number" className="premium-input h-10 px-3" value={draft.discounted_price_vnd} onChange={(event) => update("discounted_price_vnd", Number(event.target.value))} />
        </label>
        <label className="grid gap-1 text-[11px] font-bold text-[var(--muted)]">
          {ADMIN_TEXTS.plans.discount}
          <input type="number" className="premium-input h-10 px-3" value={draft.discount_percent} onChange={(event) => update("discount_percent", Number(event.target.value))} />
        </label>
        <div className="flex flex-wrap items-center gap-2 pt-5">
          <button
            type="button"
            className="inline-flex h-10 items-center gap-2 rounded-[var(--radius-control)] border border-[var(--border-color)] bg-[var(--surface)] px-3 text-[12px] font-bold text-[var(--foreground)]"
      ...
      <div className="grid gap-3 border-t border-[var(--border-subtle)] p-4 sm:grid-cols-2 xl:grid-cols-4">
        <AdminInput type="number" value={quota.chat_per_day} onChange={(event) => updateQuota("chat_per_day", Number(event.target.value))} aria-label={ADMIN_TEXTS.plans.chatQuota} placeholder={ADMIN_TEXTS.plans.chatQuota} />
        <AdminInput type="number" value={quota.ai_generations_per_day} onChange={(event) => updateQuota("ai_generations_per_day", Number(event.target.value))} aria-label={ADMIN_TEXTS.plans.generationQuota} placeholder={ADMIN_TEXTS.plans.generationQuota} />
        <AdminInput type="number" value={quota.max_documents} onChange={(event) => updateQuota("max_documents", Number(event.target.value))} aria-label={ADMIN_TEXTS.plans.documentLimit} placeholder={ADMIN_TEXTS.plans.documentLimit} />
        <AdminInput type="number" value={quota.max_file_size_mb} onChange={(event) => updateQuota("max_file_size_mb", Number(event.target.value))} aria-label={ADMIN_TEXTS.plans.fileLimit} placeholder={ADMIN_TEXTS.plans.fileLimit} />
      </div>
    </article>
  );
}
