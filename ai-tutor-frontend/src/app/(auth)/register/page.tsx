...
export default function RegisterPage() {
  ...
  const { register, googleLogin: loginWithGoogle, isLoading, error: authError } = useAuth();
  const navigate = useNavigate();

  const handleRegister = async (e) => {
    e.preventDefault();
    setLocalError('');
    if (password !== confirmPassword) { setLocalError(AUTH_TEXTS.REGISTER.PASSWORD_MISMATCH); return; }
    try {
    ...
    <div className="min-h-[100dvh] flex w-full font-sans bg-white">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center py-12 px-6 sm:px-12 relative overflow-y-auto h-full">
        <div className="w-full max-w-[420px]">
          ...
          <button type="button" onClick={() => googleLogin()} disabled={isLoading}
          ...
          <form onSubmit={handleRegister} className="flex flex-col gap-4">
            <div>
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder={AUTH_TEXTS.REGISTER.NAME_PLACEHOLDER}
            ...
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={AUTH_TEXTS.REGISTER.EMAIL_PLACEHOLDER}
                className="w-full bg-[#f8fafc] rounded-xl py-3.5 pl-4 pr-12 outline-none border-2 border-transparent focus:border-[#0052ff] focus:bg-white text-[15px] transition-all text-[#111827]"
                required disabled={isLoading} />
            </div>
          ...
          <p className="text-center text-[#6b7280] text-[14px] mt-8">
            {AUTH_TEXTS.REGISTER.HAVE_ACCOUNT} <Link to="/login" className="font-bold text-[#0052ff] hover:underline">{AUTH_TEXTS.REGISTER.LOGIN_NOW}</Link>
          </p>
        </div>
      </div>
    </div>
