...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const [doc, list] = await Promise.all([fetchDocument(documentId), fetchChatSessions(documentId)]);
        if (!active) return;
  ...
  const handleSend = async () => {
    ...
    const isActiveRequest = () =>
      activeAskRef.current?.id === requestId && !activeAskRef.current?.cancelled && !controller.signal.aborted;
    ...
    try {
      ...
      if (err?.name === "AbortError" || !isActiveRequest()) return;
      if (err instanceof QuotaError) {
      ...
      setMessages((prev) =>
        prev.concat({
          ...
          session_id: currentSessionId || "",
          role: "assistant",
          content: err.message || T.page.assistantError,
          created_at: new Date().toISOString(),
          sources: [],
        })
      );
    } finally {
      if (activeAskRef.current?.id === requestId) {
        setIsSending(false);
        abortRef.current = null;
