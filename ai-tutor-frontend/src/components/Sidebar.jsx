    ...
    <aside className="w-64 bg-[var(--sidebar-bg)] h-[100dvh] border-r border-[var(--border-color)] flex flex-col shrink-0 overflow-hidden selection:bg-[hsl(239_68%_58%/0.25)] pb-4 transition-colors duration-500">
      ...
      <div className="h-16 flex items-center justify-between gap-3 px-4 border-b border-[var(--border-color)] shrink-0">
        <Link to="/" className="flex items-center gap-3 group" onClick={() => onClose?.()}>
          ...
          <div>
            <h1 className="font-semibold text-[var(--foreground)] text-[14px] leading-none tracking-tight">
              {SIDEBAR_TEXTS.brand.title}
            </h1>
            <p className="text-[10px] text-[var(--muted-light)] font-medium mt-0.5">
      ...
      <div className="px-3 pt-5 pb-2 flex flex-col flex-1 overflow-hidden">
        ...
        <nav className="flex flex-col gap-0.5">
          <p className="text-[10px] text-[var(--muted-light)] font-bold mb-2.5 px-3 uppercase tracking-[0.12em]">
            Menu
          </p>
          {navItems.map(item => {
            ...
            return (
              ...
              >
                ...
                <span className={`material-symbols-outlined text-[18px] transition-transform duration-200 group-hover:scale-105 ${isActive ? 'icon-filled' : 'icon-thin'}`}>
                  {item.icon}
                </span>
                {item.label}
              </Link>
        ...
        {!isPro && (
          <Link to="/pricing" onClick={() => onClose?.()} className="mt-3 px-0.5 block">
            <div className="relative overflow-hidden bg-gradient-to-br from-[hsl(239_68%_58%)] to-[hsl(263_70%_55%)] rounded-3xl p-3.5 shadow-[0_2px_8px_hsl(239_68%_58%/0.20)] cursor-pointer transition-all duration-200 hover:opacity-90 hover:scale-[1.02]">
              <div
              ...
              <div className="relative z-10 flex items-center gap-3">
                ...
                <p className="text-[11px] font-bold text-white leading-none">Nâng cấp Pro</p>
              </div>
            </div>
          </Link>
        )}
